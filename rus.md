...
Что если *бы можно было* переопределить чтение и запись значений объектов?
...
Как обычно, избегайте повторений. Приведём код к такому виду:
...
Такой простой подход предоставляет некоторые преимущества перед
использованием фреймворков вроде Knockout или Angular для дата-биндинга,
например:
...
чтобы избежать проблем со скоростью работы при большом числе
...
То есть, при привязывании двух элементов к модели они обновятся только после
